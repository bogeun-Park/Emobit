...
public class CommentController {
	...
	public ResponseEntity<?> boardCreateProcess(@RequestBody @Valid CommentsCreateDto commentsCreateDto,
												@AuthenticationPrincipal CustomUser customUser) {
		if (customUser == null) {
	        return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("로그인이 필요합니다.");
	    }
		
		commentsService.createComments(customUser.getId(), commentsCreateDto);
